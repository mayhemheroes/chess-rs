...
# can't read. Thread $RUST_DEBUG_FLAGS (default: DWARF-3 + frame pointers + debuginfo) so every
...
# The cargo-fuzz crate directory. Upstream ships no fuzz/ crate, so this is an ADDITIVE
# mayhem/fuzz/ crate (harness ported from the mayhemheroes fork's original chrs-lib/fuzz).
...
# chrs-lib ships no test suite (no #[test] anywhere upstream), so there is no mayhem/test.sh.
